...
class HealHelp(HelpCommand):
    ...
    async def send_bot_help(
        ...
        embed.add_field(
            name="Need support?",
            value=f"Join the [support server](https://discord.gg/jCPYXFQekB) if you get really stuck.",
        )
            ...
            CategorySelector(
                {
                    ...
                    if key is not None
                    and key.__cog_name__.capitalize()
                    not in [
                        "Jishaku",
                        "Owner",
                        "Autorole",
    ...
    async def send_command_help(self, command: Command) -> None:
        ...
        return await self.context.send(
            ...
            .add_field(
                name="Parameters",
                value=", ".join([parameter.name for parameter in command.arguments])
                or "N/A",
                inline=True,
            )
            .add_field(
            ...
            .add_field(
                name="Usage",
                value=f'```Ruby\nSyntax: {prefix}{command.qualified_name} {" ".join([f"({parameter.name})" if not parameter.optional else f"[{parameter.name}]" for parameter in command.arguments])}```',
                inline=False,
            )
            .set_footer(
        ...
        super().__init__(
            ...
            options=[
                discord.SelectOption(
                    label="Home", description="Beginning Embed", value="home"
                ),
                *[
                    discord.SelectOption(
                        label=key, description=category.__doc__, value=key
                    )
