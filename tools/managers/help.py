...
class HealHelp(HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Cog, List[Command[Any, Callable[..., Any], Any]]]) -> None:
        ...
        view.add_item(CategorySelector({key.__cog_name__: key for key, _ in mapping.items() if key is not None and key.__cog_name__.capitalize() not in ['Jishaku', 'Owner', 'Autorole', 'Healboost', 'Onready', 'Autoresponder', 'Memberupdate']}, embed))
    ...
    async def send_command_help(self, command: Command) -> None:
        ...
        return await self.context.send(
            ...
            .add_field(name = 'Usage', value = f'>>> [**Syntax**](https://discord.gg/baUaYS2rJ6): `{prefix}{command.qualified_name} {" ".join([f"({parameter.name})" if not parameter.optional else f"[{parameter.name}]" for parameter in command.arguments])}`', inline = False)
