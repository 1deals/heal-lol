...
class HealHelp(HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Cog, List[Command[Any, Callable[..., Any], Any]]]) -> None:
        ...
        view.add_item(CategorySelector({key.__cog_name__: key for key, _ in mapping.items() if key is not None and key.__cog_name__.capitalize() not in ['Jishaku', 'Owner', 'Autorole', 'Healboost', 'Onready', 'Autoresponder']}, embed))
