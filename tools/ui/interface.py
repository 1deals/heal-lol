...
class Interface(View):
    ...
    async def lock(self, interaction: PatchedInteraction, button: Button):
        ...
        await interaction.user.voice.channel.set_permissions(
            interaction.user, connect=True, speak=True
        )
    ...
    async def reveal(self, interaction: PatchedInteraction, button: Button):
        await interaction.user.voice.channel.set_permissions(
            interaction.user, connect=True, view_channel=True
        )
        await interaction.user.voice.channel.set_permissions(
    ...
    async def claim(self, interaction: PatchedInteraction, button: Button):
        if not (
            owner := await self.bot.pool.fetchval(
                """
                SELECT owner_id
                FROM voicemaster.channels
                WHERE channel_id = $1
            ...
            return await interaction.approve(
                "This channel isn't a **VoiceMaster** channel."
            )

        if owner in [member.id for member in interaction.user.voice.channel.members]:
            return await interaction.approve(
                "The **channel** owner is already in the **channel**."
            )
    ...
    async def reject(self, interaction: PatchedInteraction, button: Button):
        if len(interaction.user.voice.channel.members) == 1:
            return await interaction.warn(
                "You cannot **disconnect** yourself from the voice channel..."
            )
        ...
        selection = discord.ui.Select(
            placeholder="Voice Members",
            options=options,
            custom_id="VM:DISCONNECT:MEMBER",
        )
        ...
        async def callback(interaction: discord.Interaction):
            ...
            if member is None:
                return await interaction.warn(
                    "I could not find the **requested member**..."
                )
            await member.move_to(None)
            return await interaction.approve(
                f"You have **disconnected** {member.mention} from the voice channel"
            )
        ...
        return await interaction.response.send_message(
            "Select a member to disconnect", view=option, ephemeral=True
        )
    ...
    async def transfer(self, interaction: PatchedInteraction, button: Button):
        if len(interaction.user.voice.channel.members) == 1:
            return await interaction.warn(
                "You cannot **transfer** the voice channel with **only yourself** in it..."
            )
        ...
        selection = discord.ui.Select(
            placeholder="Voice Members", options=options, custom_id="VM:TRANSFER:MEMBER"
        )
        ...
        async def callback(interaction: PatchedInteraction):
            ...
            return await interaction.approve(
                f"You have **transferred** the voice channel to {member.mention}"
            )
        ...
        option.add_item(selection)
        return await interaction.response.send_message(
            "Select a member to transfer the voice channel to",
            view=option,
            ephemeral=True,
        )
...
class Rename(Modal, title="Rename Channel"):
    def __init__(self):
        ...
        self.add_item(
            TextInput(
                label="Channel Name",
                placeholder="e.g. God.exe",
                custom_id="VM:RENAME:NAME",
            )
        )
    ...
    async def on_submit(self, interaction: PatchedInteraction) -> None:
        ...
        return await interaction.approve(
            f"The **channel** has been **renamed** to `{self.children[0].value}`"
        )
...
class Permit(Modal, title="Permit Member"):
    def __init__(self):
        ...
        self.add_item(
            TextInput(
                label="Discord Username or Discord ID",
                placeholder="e.g. parelite",
                custom_id="VM:PERMIT:MEMBER",
            )
        )

    async def on_submit(self, interaction: PatchedInteraction) -> None:
        member = discord.utils.get(
            interaction.guild.members, name=self.children[0].value
        ) or interaction.guild.get_member(int(self.children[0].value))
        if member is None:
            return await interaction.warn(
                "I couldn't find the **requested member** in the **guild**."
            )

        return await interaction.approve(
            f"**{member}** has been **permitted** to the **channel**."
        )
