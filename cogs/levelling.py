...
class Leveling(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        user_data = await self.bot.pool.fetchrow(
            "SELECT * FROM levels WHERE user_id = $1", user_id
        )
        ...
        if new_level > (user_data["level"] if user_data else 1):
            await self.bot.pool.execute(
                "UPDATE levels SET level = $1 WHERE user_id = $2", new_level, user_id
            )
            embed = discord.Embed(
    ...
    @commands.command(
        name="levels", description="Displays the global leaderboard for levels."
    )
    ...
    async def leaderboard(self, ctx: Context):
        ...
        for user_data in top_users:
            ...
            leaderboard.append(
                (user.display_name, user_data["level"], user_data["message_count"])
            )
        ...
        if leaderboard:
            ...
            for rank, (name, level, message_count) in enumerate(leaderboard, start=1):
                leaderboard_message += (
                    f"{rank}. {name} - Level {level} with {message_count} messages\n"
                )
            await ctx.neutral(leaderboard_message)
        else:
            await ctx.deny("There is no one on the leaderboard.")

    @commands.group(name="levelling", aliases=["level"], invoke_without_command=True)
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def levelling(self, ctx: Context):
        ...
        user_data = await self.bot.pool.fetchrow(
            "SELECT * FROM levels WHERE user_id = $1", member.id
        )

        if user_data:
            level, next_level_req = self.calculate_level(user_data["message_count"])
            embed = discord.Embed(
                description=f"{member} is at level {level}.", color=Colors.BASE_COLOR
            )
            await ctx.send(embed=embed)
        else:
            await ctx.deny(f"{member.display_name} has no messages recorded.")

    @levelling.command(name="enable", description="Enable leveling in your guild.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    @levelling.command(name="disable", description="Disable leveling in your guild.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
