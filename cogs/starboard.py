...
class Starboard(Cog):
    ...
    @starboard.command(
        name="channel", aliases=["chan"], description="Set the starboard channel."
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def starboard_channel(self, ctx: Context, *, channel: discord.TextChannel):
        ...
        return await ctx.approve(
            f"Set the **starboard channel** to: {channel.mention}."
        )

    @starboard.command(
        name="emoji", description="Set the starboard emoji, if not 🌟 will be used."
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.has_permissions(manage_channels=True)
    async def starboard_emoji(self, ctx: Context, *, emoji: str):
        try:
            await self.bot.pool.execute(
                "INSERT INTO starboard (guild_id, emoji) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET emoji = $2",
                ctx.guild.id,
                emoji,
            )
    ...
    async def starboard_threshold(self, ctx: Context, *, threshold: int):
        await self.bot.pool.execute(
            "INSERT INTO starboard (guild_id, threshold) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET threshold = $2",
            ctx.guild.id,
            threshold,
        )
            ...
            try:
                if message_id in self.skull_messages:
                    ...
                    await sent_message.edit(
                        content=f"#{skull_reactions} {emoji}", embed=embed
                    )
                else:
