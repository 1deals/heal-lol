...
class Roleplay(Cog):
    ...
    @command(name="hug", description="Hugs a user.")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    embed = discord.Embed(
                        description=f"Aww {ctx.author.mention} **hugged** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="kiss", description="Kisses a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def kiss(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"Aww {ctx.author.mention} **kissed** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="slap", description="Slaps a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def slap(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **slapped** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="shoot", description="Shoots a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def shoot(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **shot** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="stare", description="Stare at a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def stare(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **stares** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="wave", description="Wave at a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def wave(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **waves to** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="poke", description="Poke a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def poke(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **poked** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="wink", description="Wink at a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def wink(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **winked at** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="tickle", description="Tickle a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def tickle(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **tickled** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)

    @command(name="feed", description="Feed a user.")
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def feed(
        self, ctx: Context, *, user: Union[discord.Member, discord.User] = None
    ):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    gif_url = data["results"][0]["url"]
                    embed = discord.Embed(
                        description=f"{ctx.author.mention} **feeds** {user.name}",
                        color=Colors.BASE_COLOR,
                    )
                    embed.set_image(url=gif_url)
                    await ctx.send(embed=embed)
