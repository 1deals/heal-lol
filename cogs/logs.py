...
class Logs(commands.Cog):
    ...
    @logs.command(
        name="joins", aliases=["on_join", "join"], description="Setup join logs."
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @has_permissions(administrator=True)
    async def logs_joins(self, ctx: Context, *, channel: discord.TextChannel = None):
        ...
        data = await self.bot.pool.fetchrow(
            "SELECT joinlogschannel FROM logging WHERE guild_id = $1", ctx.guild.id
        )
        ...
        await self.bot.pool.execute(
            "INSERT INTO logging (guild_id, joinlogschannel) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET joinlogschannel = $2",
            ctx.guild.id,
            channel.id,
        )
    ...
    async def on_member_join(self, member: discord.Member):
        res = await self.bot.pool.fetchrow(
            "SELECT * from logging WHERE guild_id = $1", member.guild.id
        )
    ...
    @logs.command(name="leave", aliases=["on_leave"], description="Setup leave logs")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @has_permissions(administrator=True)
    async def logs_leave(self, ctx: Context, *, channel: discord.TextChannel):
        ...
        data = await self.bot.pool.fetchrow(
            "SELECT leavelogschannel FROM logging WHERE guild_id = $1", ctx.guild.id
        )
        ...
        await self.bot.pool.execute(
            "INSERT INTO logging (guild_id, leavelogschannel) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET leavelogschannel = $2",
            ctx.guild.id,
            channel.id,
        )
    ...
    @logs.command(
        name="message", aliases=["messages"], description="Setup message logs"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @has_permissions(administrator=True)
    async def logs_message(self, ctx: Context, *, channel: discord.TextChannel):
        ...
        data = await self.bot.pool.fetchrow(
            "SELECT messagelogschannel FROM logging WHERE guild_id = $1", ctx.guild.id
        )
        ...
        await self.bot.pool.execute(
            "INSERT INTO logging (guild_id, messagelogschannel) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET messagelogschannel = $2",
            ctx.guild.id,
            channel.id,
        )
    ...
    async def on_message_delete(self, message: discord.Message):
        res = await self.bot.pool.fetchrow(
            "SELECT * from logging WHERE guild_id = $1", message.guild.id
        )
        if res:
            ...
            if channel:
                embed = discord.Embed(
                    title=f"", description=message.content, color=Colors.BASE_COLOR
                )
                embed.set_author(
                    name=f"{message.author.name} deleted a message in #{message.channel}.",
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        if before.content == after.content:
            return
        res = await self.bot.pool.fetchrow(
            "SELECT * from logging WHERE guild_id = $1", before.guild.id
        )
        if res:
            channel_id = res["messagelogschannel"]
            ...
            if channel:
                ...
                embed.set_author(
                    name=f"{before.author.name} edited a message in #{before.channel}",
    ...
    @logs.command(name="voice", aliases=["vc"], description="Setup voice logs")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @has_permissions(administrator=True)
    async def logs_voice(self, ctx: Context, *, channel: discord.TextChannel = None):
        ...
        await self.bot.pool.execute(
            "INSERT INTO logging (guild_id, voicelogschannel) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET voicelogschannel = $2",
            ctx.guild.id,
            channel.id,
        )
        return await ctx.approve(
            f"Voice chat logs will now be sent to {channel.mention}."
        )
        ...
        if res:
            ...
            if channel:
                ...
                if before.channel is None and after.channel is not None:
                    ...
                    embed.set_author(
                        name=f"{member.name} connected to a voice channel.",
                    ...
                    embed.set_author(
                        name=f"{member.name} disconnected from a voice channel.",
                        icon_url=member.avatar.url,
                    )
                    ...
                    embed.set_author(
                        name=f"{member.name} moved voice channels.",
                        icon_url=member.avatar.url,
                    )
                ...
                if not before.self_mute and after.self_mute:
                    ...
                    embed.set_author(
                        name=f"{member.name} was muted.", icon_url=member.avatar.url
                    )
                    ...
                    embed.set_author(
                        name=f"{member.name} was unmuted.", icon_url=member.avatar.url
                    )
                ...
                if not before.self_deaf and after.self_deaf:
                    ...
                    embed.set_author(
                        name=f"{member.name} was deafend.", icon_url=member.avatar.url
                    )
                    ...
                    embed.set_author(
                        name=f"{member.name} was undeafend.", icon_url=member.avatar.url
                    )
    ...
    async def logs_list(self, ctx: Context):
        ...
        embed.add_field(
            name="Joins",
            value=(
                f"{Emojis.APPROVE} {ctx.guild.get_channel(join_channel).mention}"
                if join_channel
                else f"{Emojis.DENY} Joins"
            ),
            inline=False,
        )
        ...
        embed.add_field(
            name="Leaves",
            value=(
                f"{Emojis.APPROVE} {ctx.guild.get_channel(leave_channel).mention}"
                if leave_channel
                else f"{Emojis.DENY} Leaves"
            ),
            inline=False,
        )
        ...
        embed.add_field(
            name="Messages",
            value=(
                f"{Emojis.APPROVE} {ctx.guild.get_channel(message_channel).mention}"
                if message_channel
                else f"{Emojis.DENY} Messages"
            ),
            inline=False,
        )
        ...
        embed.add_field(
            name="Voice",
            value=(
                f"{Emojis.APPROVE} {ctx.guild.get_channel(voice_channel).mention}"
                if voice_channel
                else f"{Emojis.DENY} Voice"
            ),
            inline=False,
        )
