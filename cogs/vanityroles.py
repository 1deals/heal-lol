...
class Vanityroles(commands.Cog):
    ...
    async def on_presence_update(self, _, member):
        data = await self.bot.pool.fetchrow(
            "SELECT * FROM vanityroles WHERE guild_id = $1", member.guild.id
        )
            ...
            if string in current_activity:
                if cached is None:
                    if role not in member.roles:
                        ...
                        embed = discord.Embed(
                            description=f"Thank you for repping **{string}**, {member.mention}!",
                            color=Colors.BASE_COLOR,
                        )
                        await channel.send(embed=embed)
                    await self.bot.cache.set(
                        f"vanityroles-{member.id}", True, timeout=14400
                    )
            else:
    ...
    @vanityroles.command(
        name="channel", description="Set the channel to send award logs."
    )
    @commands.has_permissions(manage_guild=True)
    async def vanityroles_channel(
    ...
    @vanityroles.command(name="role", description="Set the role to award members.")
    @commands.has_permissions(manage_guild=True)
    ...
    @vanityroles.command(name="test", description="Test the vanityrole system.")
    @commands.has_permissions(manage_guild=True)
    async def vanityroles_test(self, ctx: Context):

        data = await self.bot.pool.fetchrow(
            "SELECT role_id, channel_id FROM vanityroles WHERE guild_id = $1",
            ctx.guild.id,
        )
        ...
        if not drole or not dchannel:
            ...
            emb = Embed(
                description=f"{ctx.author.mention} Thanks for repping us! <:pocoyo_shrug:1244434508334235658>",
                color=Colors.BASE_COLOR,
            )
            await ctx.approve(
                f"Successfully sent vanity message and gave the role to you!"
            )
    ...
    async def vanityroles_disable(self, ctx: Context):
        ...
        data = await self.bot.pool.fetchrow(
            "SELECT guild_id FROM vanityroles WHERE guild_id = $1", ctx.guild.id
        )
