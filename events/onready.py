...
class onready(Cog):
    ...
    async def on_ready(self): 
      ...
      embed = discord.Embed(color=Colors.BASE_COLOR, description=f"{online} {self.bot.user.name} is online, serving `{len(self.bot.guilds): ,}` guilds, with about `{len(set(self.bot.get_all_members())): ,}` members at `{round(self.bot.latency * 1000)}ms`")
